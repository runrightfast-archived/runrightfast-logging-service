...
(function() {
	...
	function LoggingService() {
		...
		this.handleInvalidEvent = function(event, error) {
			var badEvent = {
				ts : new Date(),
				event : 'invalidEvent',
				badEvent : event,
	 ...
	 *            <li>ts - OPTIONAL - either a Number representing the EPOCH time for
	 *            the event, a string representing an RFC2822 or ISO 8601 date, or a Date. If not specified, then the current time will be
	 *            set.
	 *            </ul>
	...
	module.exports = function(options) {
		...
		if (options) {
			...
			configureInvalidEventListener(options.invalidEventListener);
		}
